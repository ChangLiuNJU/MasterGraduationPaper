...
陀螺仪传感器，又称为角速度传感器，用以测量物体的旋转角度。如图\ref{fig:gyroscope} 所示，陀螺仪由一个绕着可旋转的轴旋转的圆环或者圆盘组成。当圆环旋转时，其可以表示物体的三维朝向。陀螺仪传感器具有用途广泛。由于陀螺仪可以提供准确的方位和水平信息，其被用于航海、航空和宇航事业中。在导弹、卫星等对于运行轨道有严格要求的系统中，陀螺仪可以进行运动轨道的校正。在无人机领域，陀螺仪用于保证无人机的云台稳定。现代智能手机和平板中也嵌入了陀螺仪传感器，其提供了新型的交互方式和娱乐方式。当用户用智能手机进行拍照时，陀螺仪可以在按下快门的瞬间计算出手机的抖动情况，从而进行拍照防抖。陀螺仪还可以辅助GPS进行手机的定位，当手机接收不到GPS信号时，手机通过陀螺仪计算偏航，从而实现辅助导航。
...
基于定位设施的室内定位方案大多利用无线信号进行室内定位，例如无线射频信号、Wifi 信号和蓝牙信号，在室内环境中进行对人或物体的定位或追踪。许多定位算法例如Fingerprint\cite{fp1,fp2,fp3}和LANDMARC\cite{landmarc1,landmarc2}
...
为了评估FootStep-Tracker，我们邀请了5个用户进行测试。他们穿着FootStep-Tracker，沿着如图\ref{fig:path}所示的红色实线的路线行动。在六次转弯之后，他乘坐另一个电梯离开当前室内环境。首先，FootStep-Tracker 并不能确定哪个电梯为用户的确切位置。图中的蓝色虚线显示了可能的用户位置。首先，用户走了一小段距离，然后右转，直走了一段很长的距离。此时，FootStep-Tracker通过贪食蛇策略\footnote{https://en.wikipedia.org/wiki/Snake\_(video\_game)}可以排除了虚线所示的几个用户行为轨迹。图\ref{fig:eva_case_study} (a) 显示了一个用户走路的步数与定位误差的关系。在开始时，FootStep-Tracker由于还没有确定用户的可能位置，具有较大的定位误差。随着用户继续行走，用户的确切位置在六步之后确定，定位误差锐减到1m 左右。图\ref{fig:eva_case_study} (b) 显示了转弯数和定位误差的关系。由于第一个转弯时，用户的确切位置还没有确定，并不具有参考价值。我们从第二个转弯开始，发现随着转弯数目的增加，定位误差可以明显减少，此外双脚校正也可以有效的减少误差。图\ref{fig:eva_case_study} (c) 显示了平均误差在大约1m 左右。
...
SensorTag是德州仪器公司开发的业界首款蓝牙开发套件。图\ref{fig:sensortag_arch} 所示为SensorTag的系统架构，SensorTag将多种传感器集成在基于CC2541的单板上，其中包括红外温度传感器、湿度传感器、气压传感器、加速度计、陀螺仪和磁力计。支持开发持多种平台，包括PC、Android 和IOS平台，并可以直接通过智能手机进行固件的版本更新。
...
图\ref{fig:imp_scene}所示为系统的场景图。本系统为一种基于脚步感知的无锚节点室内定位系统，主要包括SnesorTag和GALAXY S5智能手机，二者通过蓝牙低功耗进行数据交互。SensorTag上搭载加速度计、陀螺仪组成的IMU 模块和BLE蓝牙通信模块。智能手机搭载室内地图，接收来自SensorTag传输的传感器数据，通过进行分析处理，定位用户位置并与用户友好的交互。
...
界面左上部为计步器和行为识别部分。当用户在室内活动时，Activity实时显示行为分类器检测出的用户的行为，包括静止站立(Stand Still)，走路(Walk)，上楼梯(UST)，下楼梯(DST)，上电梯(EA)和下电梯(ED)。当用户在走路时，Step实时显示用户走的步数。
...
本章中，我们阐释了基于脚步感知的无锚节点定位系统实现。我们采用德州仪器公司生产的CC2541 SensorTag芯片与三星公司生产的GALAXY S5作为实现的硬件平台，采用JAVA语言和Android平台作为实现的软件平台。在智能手机上，我们实现了FootStep-Tracker App，当用户在室内实时行动时，App可以显示出用户的步数、当前行为、行为轨迹，以及在地图中用户的确切位置。
...
近年来，随着物联网技术、可穿戴计算和室内定位技术的蓬勃发展，人们的生活、工作、娱乐方式都在逐步的改变。物联网、可穿戴计算、室内定位三大技术在包括商业、运输、交通、能源、家居等许许多多的领域都给人们提供了更加方便、安全和高效的生产方式。在学术界和工业界，大多数的室内定位方案都是利用辅助定位设施，例如WiFi Ap和蓝牙Beacon 等对用户携带的智能设备进行定位，这些方案的精度受到设备的部署密度影响，具有一定的局限性。其部署成本也相对较高，单个定位节点单价在百元左右。此外，在没有提前部署定位辅助设施的环境中，其无法完成定位。为了应对这种情况，在学术界，也有利用手机的传感器等对用户进行室内导航的方案，但定位效果往往受到手机不同摆放姿态等因素的影响，导致定位误差较大或部分依赖于定位设施的辅助。
...
其次，本位提出了基于脚步感知的无锚节点室内定位系统的实现。我们采用德州仪器公司生产的SensorTag和三星公司生产的GALAXY S5对系统进行了实现。我们采用Java语言在Android平台上实现了FootStep-Tracker应用程序。其可以实时的显示SensorTag采集到的传感器数据、用户的步数、用户的行为轨迹以及用户在室内地图中的位置。
