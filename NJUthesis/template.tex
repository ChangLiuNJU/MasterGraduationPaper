...
为了更近一步的减少累积误差，FootStep-Tracker将两个传感器分别嵌入用户的左右两个鞋子中，并分别的进行步长估算。由于在某一特定时刻，两个脚的累计步长，即两脚的位置不会相差太远，我们给出双脚校正的机制。当两个脚分别的累计步长相差达到1m以上时，我们将他们的平均值作为当前的用户行走距离，并重置双脚的累计步长。
...
为了定位用户和更进一步的追踪和导航，我们采用了贪食蛇策略\cite{snakegame}。如图\ref{fig:Anakonda} (a) 所示，用户乘坐电梯来到环境中，然后右转走了一小段距离。此时根据行走轨迹和电梯的位置，有三个可能的用户位置。随着用户行走，到了如图 (b) 的时刻，由于地图拓扑环境的限制，可能的行走轨迹 (2) 和 (3) 被排除掉了，从而筛选出确切的用户行为轨迹(1)。
