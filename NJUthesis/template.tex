...
\documentclass[dvipdfm, twoside, master]{NJUthesis}
% 可选参数：
...
可穿戴设备（Wearable computer）为可穿戴于身上出外进行活动的微型电子设备\footnote{https://en.wikipedia.org/wiki/Wearable\_computer}。
...
参考地点估算器根据采集到的用户脚部传感器数据，通过行为分类器，得到用户当前的行为。当用户进行一些特定的行为时，例如上下电梯，上下楼梯，我们就可以得到用户当前在室内地图中的可能地点，即所说的参考地点，例如电梯和楼梯。进一步，利用室内地图中的拓扑限制，利用类似于Snake Game\footnote{https://en.wikipedia.org/wiki/Snake\_(video\_game)} 的筛选策略，就可以相对的用户运动轨迹就可以确定为室内地图中绝对的运动轨迹。
...
本文第三章基于脚步感知的无锚节点室内定位系统，详细给出研究中的问题和解决方案。对FootStep-Tracker 的几个部分行为分类器、脚部切割、步长估算器和方向估算器的工作方法进行详细阐释，并给出了实验及结果分析。

本文第四章基于脚步感知的无锚节点室内定位系统实现，介绍了在真实环境下利用德州仪器公司生产的SensorTag\cite{sensortag}和Android 智能手机对本系统进行的具体实现。
...
现代智能移动设备，例如智能手机、平板电脑中大多搭载了惯性传感器，用以帮助智能手机软件开发者对智能手机用户提供更加多样化的服务，例如摇一摇搜索周边的朋友功能、平衡球游戏等。惯性传感器(Inertial measurement unit, IMU)是用来测量受力、角速度，有时也测量周围磁场的电子设备，主要包含加速度计、陀螺仪和磁力计。
下面简单介绍与本文相关的两类传感器：加速度传感器和陀螺仪传感器，简要介绍他们的工作原理以及应用。

\subsection{加速度传感器}

加速度传感器是测量固有加速度的电子设备。固有加速度是指包含了物体所受到的重力的加速度，即当传感器芯片静止摆放时，其沿重力方向的读数为重力加速度g。
高精的加速度计制作工艺十分复杂。如图\ref{fig:accelerometer}所示，从概念上讲，单轴的加速度计的结构为固定在弹簧上的阻尼块。当加速度计加速时，阻尼块的惯性致使弹簧产生形变，从而反应为加速度计的读数。三轴加速计的结构即为三个相互垂直的单轴加速度计组合而成。
加速度计具有多种用途。高精度的加速度计被广泛用于航空和航天业中，用以辅助导航。现代平板电脑和智能手机中嵌入的加速度传感器给用户带来了新的交互方式，例如智能手机的屏幕可以根据加速度计感知的重力的方向自动旋转到正确的角度，即横屏、竖屏自动切换。腾讯公司的微信摇一摇功能也是利用加速度计感应手机的摇晃。此外，在游戏中，加速度计可以通过检测手机屏幕的倾斜角度，从而提供新的娱乐方式，例如成为方向盘等功能。

\begin{figure}[!ht]
\centering
\includegraphics[width = 0.7\textwidth]{Figure/accelerometer.jpg}
\caption{加速度计原理示意图}
...
陀螺仪传感器，又称为角速度传感器，用以测量物体的旋转角度。如图\ref{fig:gyroscope} 所示，陀螺仪由一个绕着可旋转的轴旋转的圆环或者圆盘组成。当圆环旋转时，其可以表示物体的三维朝向。陀螺仪传感器具有用途广泛。由于陀螺仪可以提供准确的方位和水平信息，其被用于航海、航空和宇航事业中。在导弹、卫星等对于运行轨道有严格要求的系统中，陀螺仪可以进行运动轨道的校正。在无人机领域，陀螺仪用于保证无人机的云台稳定。现代智能手机和平板中也嵌入了陀螺仪传感器，其提供了新型的交互方式和娱乐方式。当用户用智能手机进行拍照时，陀螺仪可以在按下快门的瞬间计算出手机的抖动情况，从而进行拍照防抖。陀螺仪还可以辅助GPS进行手机的定位，当手机接收不到GPS 信号时，手机通过陀螺仪计算偏航，从而实现辅助导航。

\begin{figure}[!ht]
\centering
\includegraphics[width = 0.5\textwidth]{Figure/gyroscope.jpg}
\caption{陀螺仪原理示意图}
\label{fig:gyroscope}
\end{figure}
...
基于定位设施的室内定位方案大多利用无线信号进行室内定位，例如无线射频信号、Wifi 信号和蓝牙信号，在室内环境中进行对人或物体的定位或追踪。在学术界，许多定位算法例如Fingerprint\cite{fp1,fp2,fp3,fp4}，LANDMARC\cite{landmarc1,landmarc2}，AOA\cite{aoa1}等都被广泛接受。基于定位设施的室内定位方案主要分为以下几类，基于参考锚节点的定位，基于空间指纹的定位和基于信号模型的定位。

\subsection{基于参考锚节点的定位}

基于参考锚节点的定位是指采集用户所携带的无线设备发出的无线信号，以及提前部署在室内环境中的无线锚节点发出的无线信号，将二者进行比对，进而通过已知的无线锚节点位置来估算用户的当前位置。

Ni\cite{landmarc1}等人提出的LANDMARC算法是基于无线射频识别（Radio-frequency identification，RFID）系统的室内定位系统。其通过提前部署的参考标签位置，估算出未知目标标签位置。其采用的为最近邻居法（k-Nearest Neighbors，KNN)算法，即采用RFID阅读器读取到的RFID标签，包括已知位置的锚节点标签和未知位置的目标标签的信号强度作为特征值，选择与目标标签的特征最为相近的若干锚节点标签，取它们位置的加权平均值作为目标标签的位置。

\subsection{基于空间指纹的定位}

基于空间指纹的定位是指，当定位目标携带无线信号在室内环境中移动时，采集目标在不同已知位置的无线信号信息，例如信号强度、相位等，作为目标在各个位置的指纹信息。

Yang\cite{fp4}等人提出的LiFS就是基于空间指纹的室内定位系统。图\ref{fig:fp}为其系统架构图，他们将WiFi节点部署在面积为1600平米的室内环境中。在训练部分，他们通过在室内地图中各个位置采集的WiFi信号的RSS数据构成指纹数据库。在定位时，通过将实时采集到的RSS数据与指纹数据库进行匹配，从而估算出用户的位置。

\begin{figure}[!ht]
\centering
\includegraphics[width = 0.9\textwidth]{Figure/fp.jpg}
\caption{LiFS系统架构图}
\label{fig:fp}
\end{figure}
...
在许多室内环境中，并不具备部署或大量部署定位锚节点设施的条件，或者部署起来成本过高，于是无锚节点的室内定位方案应运而生。当前最前沿的无锚节点的室内定位方案，特别是室内行人导航主要是利用用户身上携带的智能设备中的惯性传感器，通过传感器感知的加速度，角速度，磁场等物理信号，估算用户在室内环境中的位置。在学术界也提出了许多室内定位方案，例如基于航位推测法、脚步感知等方案，可以有效的解决在没有定位锚节点的情况下的室内定位。

\subsection{航位推测法}

航位推测法（Dead-reckoning）是一种根据目标之前确定的、已知的位置来估算目标当前位置的方法。
...
\caption{FootStep-Tracker系统框架。输入传感器数据和室内地图，FootStep-Tracker 输出用户在给定地图中的实时位置。}
...
如图\ref{fig:axes}，为了方便下文讨论，我们首先给出SensorTag部署在鞋子中时，其坐标系。我们将SnesorTag 的x 轴沿着脚的左右方向摆放，y轴沿着脚的前后方向摆放，z轴沿着垂直方向摆放。我们记三轴的加速度分别为$a_x$，$a_y$和$a_z$，三轴陀螺仪角速度为$g_x$，$g_y$和$g_z$。
...
这是因为当用户静止站立时，加速度z 轴的方向与垂直方向一致，即为重力的相反方向。
...
而上楼梯时，加速度则具有最小的方差。
图\ref{fig:Moving Pattern} (e) 所示为用户乘坐电梯上行时的加速度z轴读数。图中的红色方框中显示，z 轴加速度首先小于重力加速度一段时间，然后等于重力加速度，最后其大于重力加速度一段时间。
这是因为当电梯向上加速以上升时，在电梯中的用户首先是在超重状态下，在此时，重力加速度“变大”，其z 轴方向的加速度读数为一个更小的负数。
然后的一段时间，电梯以常数速度上升，此时用户与电梯相对静止，受到的z轴加速度为重力加速度。
最后，电梯减速直至静止。
此时，用户在失重状态下，此时重力加速度“变小”，其z轴加速度为一个相对于重力加速度较大的负数。
图\ref{fig:Moving Pattern} (f)显示了电梯下行时的加速度z 轴读数。此时读数的变化与电梯上行时正好相反，用户首先失重，最后超重。读数较重力先变大，再相等，最后变小。
...
通常情况下，人类的步频在1Hz到3Hz之间。电梯的加速和减速过程通常持续2 秒左右。我们用一个窗口大小为40的滑动窗口来分割数据。每个窗口包含2秒内的数据，以保证整个窗口可以包含整个电梯加速减速和走路的过程。
...
如图\ref{fig:cdf_activity} (a) 所示，由两组动作的窗口方差统计结果来看，这两组动作窗口方差的分布之间有一个明显的分割。我们利用一个为9.064 的方差阈值去区分这两组不同的窗口。
...
加速度的y轴几乎与人体前进的方向一致，我们的脚部切割主要是利用加速度y 轴，辅助以加速度x轴和z轴。
...
为了防止由与走路相似的周期性行为，例如摆腿导致的误切割，我们不仅仅检测前进方向y轴上的“静止区间”，同样检测x 轴和z轴上的“静止区间”。
...
图\ref{fig:phase} (a) 所示为用户走路时脚部的运动轨迹。如图所示，加速度y轴并不总是水平的，我们将y 轴的方向投影在水平面上，并记为脚部方向（foot direction），记y轴方向与脚部方向的夹角为$\theta$。
...
在静止时间（rest time，$T_r$）时，$a_y,g_x$再次为0。我们在给定的数据段中，着地时间后，寻找第一个$a_y,g_x$为0 的点作为静止时间。
...
图\ref{fig:after_calibration}所示为用户一个步长为1.23m 的脚步，步长估算器的估算结果。
...
图\ref{fig:gzl_90_walking}所示为用户在一个90度左转时，传感器采集到的数据。图中红色方框为转弯时采集到的数据，对于一个90 度的左转来说，左脚有两步导致了这次左转，右脚在左脚的这两步之间有一步。此外，当用户向前行走时，由于脚部轻微的左右摆动和八字脚的关系，$g_z$也有轻微的波动。
...
为了定位用户和更进一步的追踪和导航，我们采用了贪食蛇策略\footnote{https://en.wikipedia.org/wiki/Snake\_(video\_game)}。 如图\ref{fig:Anakonda} (a) 所示，用户乘坐电梯来到环境中，然后右转走了一小段距离。此时根据行走轨迹和电梯的位置，有三个可能的用户位置。随着用户行走，到了如图 (b) 的时刻，由于地图拓扑环境的限制，可能的行走轨迹 (2) 和 (3) 被排除掉了，从而筛选出确切的用户行为轨迹(1)。
...
\noindent\textbf{硬件：}我们的系统包含由德州仪器公司生产的CC2541 SensorTag\cite{sensortag}和一台三星Galaxy S5 安卓智能手机。
...
为了评估行为分类器，我们首先采集了上文中提到的6 个行为的传感器数据。
对于每一个行为，我们收集了来自3个用户的500个窗口的加速度和陀螺仪数据。
然后我们利用这些数据来训练HMM模型以及确定决策树的阈值。
我们将训练好的分类器在线为之前的3 个用户和5个新的用户进行行为分类。
他们在环境中走路或者是上下楼梯、电梯。
...
为了评估方向估算器，我们邀请了8位用户，从-180度到+180 度，以30度为粒度，每次转弯10 次。图\ref{fig:eva_step_direction} (a) 显示了角度误差。
...
为了评估FootStep-Tracker，我们邀请了5个用户进行测试。他们穿着FootStep-Tracker，沿着如图\ref{fig:path}所示的红色实线的路线行动。在六次转弯之后，他乘坐另一个电梯离开当前室内环境。首先，FootStep-Tracker 并不能确定哪个电梯为用户的确切位置。图中的蓝色虚线显示了可能的用户位置。首先，用户走了一小段距离，然后右转，直走了一段很长的距离。此时，FootStep-Tracker 通过贪食蛇策略\footnote{https://en.wikipedia.org/wiki/Snake\_(video\_game)} 可以排除了虚线所示的几个用户行为轨迹。图\ref{fig:eva_case_study} (a) 显示了一个用户走路的步数与定位误差的关系。在开始时，FootStep-Tracker 由于还没有确定用户的可能位置，具有较大的定位误差。随着用户继续行走，用户的确切位置在六步之后确定，定位误差锐减到1m 左右。图\ref{fig:eva_case_study} (b) 显示了转弯数和定位误差的关系。由于第一个转弯时，用户的确切位置还没有确定，并不具有参考价值。我们从第二个转弯开始，发现随着转弯数目的增加，定位误差可以明显减少，此外双脚校正也可以有效的减少误差。图\ref{fig:eva_case_study} (c) 显示了平均误差在大约1m 左右。
...
在于脚步感知的无锚节点室内定位系统的基础上，我们对FootStep-Tracker 做了实际的系统实现。不失一般性，我们采用德州仪器公司生产的SensorTag\cite{sensortag}作为嵌入用户鞋子内的传感器芯片，采用三星公司生产的Glaxy S5智能手机作为数据的处理和界面展示平台，对系统进行了功能上的实现和效果验证。

本章首先介绍了系统实现的硬件与软件平台，包括本系统采用的硬件平台与调整的硬件参数。在此基础上，介绍了系统设计的各个部分，包括其场景结构、数据流和工作原理。最后，对系统在Android 平台上的实现效果进行展示和效果验证。

\section{实现平台}

\subsection{硬件平台}

如图\ref{fig:hardware}所示，FootStep-Tracker的实现采用了德州仪器公司生产的SensorTag\cite{sensortag} 作为嵌入用户鞋子中的传感器芯片。三星公司生产的Galaxy S5\footnote{http://www.samsung.com/cn/promotions/s5/index.html} 智能手机作为数据的处理和界面展示平台。
...
SensorTag是德州仪器公司开发的业界首款蓝牙开发套件。图\ref{fig:sensortag_arch} 所示为SensorTag的系统架构，SensorTag将多种传感器集成在基于CC2541 的单板上，其中包括红外温度传感器、湿度传感器、气压传感器、加速度计、陀螺仪和磁力计。支持开发持多种平台，包括PC、Android 和IOS 平台，并可以直接通过智能手机进行固件的版本更新。

SensorTag采用蓝牙低功耗（Bluetooth low energy, BLE）\footnote{https://developer.bluetooth.org/TechnologyOverview/Pages/BLE.aspx}作为数据传输协议。BLE较传统的蓝牙来说，旨在保持通讯范围的同时，提供更少的耗电量。BLE 在2006 年首先由Nokia提出，当时被称为\textbf{Wibree}。 随后在2010年，其被并入蓝牙4.0协议。

\begin{figure}[!ht]
\centering
\includegraphics[width=0.9\textwidth]{Figure/sensortag_arch.png}
\caption{sensortag系统架构图\cite{sensortag}}
\label{fig:sensortag_arch}
\end{figure}

\subsection{软件平台}

FootStep-Tracker的实现采用JAVA语言\footnote{https://www.java.com/zh\_CN}。JAVA 为Sun Microsystems公司推出的一种面向对象设计语言，其具有跨平台、高效、安全等优势，其广泛应用于个人电脑、数据中心、超级计算机等平台，具有世界上最大的开发者群体。

FootStep-Tracker通过Android\footnote{https://www.android.com} 平台实现在于智能手机上。Android平台由Google公司推出，其基于Linux平台，具有开放性强、交互性好，开发便捷等优势，现已与IOS平台成为世界上的两大移动操作系统平台。

\section{系统设计}

图\ref{fig:imp_scene}所示为系统的场景图。本系统为一种基于脚步感知的无锚节点室内定位系统，主要包括SnesorTag和GALAXY S5智能手机，二者通过蓝牙低功耗进行数据交互。SensorTag上搭载加速度计、陀螺仪组成的IMU 模块和BLE 蓝牙通信模块。智能手机搭载室内地图，接收来自SensorTag传输的传感器数据，通过进行分析处理，定位用户位置并与用户友好的交互。
...
\begin{enumerate}
  ...
  \item 智能手机的Android应用中加载室内地图。室内地图中包含室内环境包括道路和房间的详细拓扑信息，电梯、楼梯的具体位置与大小。
  \item 智能手机的蓝牙通信模块与SensorTag的蓝牙通信模块进行配对。
  ...
  \item 当用户正在走路时，智能手机的处理器通过收集的传感器数据，估算出用户的行为轨迹。
  \item 智能手机结合估算出的运动轨迹和参考位置，根据室内地图的拓扑信息，估算出用户的具体位置，并通过智能手机的显示屏显示。
\end{enumerate}
...
图\ref{fig:imp_how}所示为本系统的工作原理图。其中，智能手机通过加速度和陀螺仪数据进行用户的行为分类。
当用户在走路时，以每一步为单位，计算用户的行走距离和行走方向，结合成为用户的行为轨迹。
当用户在上下楼梯或是乘坐电梯时，在室内地图中寻找楼梯或电梯的位置，作为参考位置。
结合行走路线与参考地点，在室内地图中根据参考地点确定用户行走路线的确切位置，从而完成用户的精确定位。
...
如图\ref{fig:app_main}所示，在完成SensorTag与智能手机的蓝牙配对后，智能手机展示如下的Andorid App 定位主界面。

界面左上部为计步器和行为识别部分。当用户在室内活动时，Activity实时显示行为分类器检测出的用户的行为，包括静止站立(Stand Still)，走路(Walk)，上楼梯(UST)，下楼梯(DST)，上电梯(EA)和下电梯(ED)。当用户在走路时，Step 实时显示用户走的步数。

界面右上部为用户的行为轨迹。当用户在室内环境中走路时，通过步长估算器和方向估算器估算出来的行动距离和行动方向实时合成而成为行为轨迹。

界面中上部为蓝牙的RSSI和传感器的读数部分。其实时的显示了SnesorTag所采集的加速度和陀螺仪数据。此外，为了查看蓝牙是否实时保持连接，其还显示了蓝牙的RSSI数据。

界面下半部为室内地图以及用户在室内地图中的实时位置。当用户在室内地图中行动时，地图中实时显示用户的位置，并描绘出用户之前的行为轨迹。

\begin{figure}[!ht]
\centering
\includegraphics[width=0.4\textwidth]{Figure/app_main.png}
\caption{Android应用定位界面}
\label{fig:app_main}
\end{figure}

\section{本章小结}

本章中，我们阐释了基于脚步感知的无锚节点定位系统实现。我们采用德州仪器公司生产的CC2541 SensorTag 芯片与三星公司生产的GALAXY S5作为实现的硬件平台，采用JAVA语言和Android 平台作为实现的软件平台。在智能手机上，我们实现了FootStep-Tracker App，当用户在室内实时行动时，App可以显示出用户的步数、当前行为、行为轨迹，以及在地图中用户的确切位置。
...
近年来，随着物联网技术、可穿戴计算和室内定位技术的蓬勃发展，人们的生活、工作、娱乐方式都在逐步的改变。物联网、可穿戴计算、室内定位三大技术在包括商业、运输、交通、能源、家居等许许多多的领域都给人们提供了更加方便、安全和高效的生产方式。在学术界和工业界，大多数的室内定位方案都是利用辅助定位设施，例如WiFi Ap 和蓝牙Beacon 等对用户携带的智能设备进行定位，这些方案的精度受到设备的部署密度影响，具有一定的局限性。其部署成本也相对较高，单个定位节点单价在百元左右。此外，在没有提前部署定位辅助设施的环境中，其无法完成定位。为了应对这种情况，在学术界，也有利用手机的传感器等对用户进行室内导航的方案，但定位效果往往受到手机不同摆放姿态等因素的影响，导致定位误差较大或部分依赖于定位设施的辅助。

本文首先提出了FootStep-Tracker，一种无锚节点的室内定位系统。本系统完全利用嵌入在用户脚部的传感器芯片，无需任何定位辅助设施对用户进行室内定位。我们使用传感器芯片上的加速度计和陀螺仪感知用户在室内环境中的行为，例如走路、上下楼梯、上下电梯等。当用户在走路时，我们根据估算的用户行走距离和用户行走方向，估算出用户的行为轨迹。而当用户在上下楼梯或者乘坐电梯时，本系统将楼梯或电梯作为参考位置，帮助将相对的用户行为轨迹确定为绝对的用户在室内环境中的位置。此外，本系统还利用室内空间的拓扑结构对估算出的可能的多条用户行为轨迹进行过滤筛选，进而收敛为准确的行为轨迹。

其次，本位提出了基于脚步感知的无锚节点室内定位系统的实现。我们采用德州仪器公司生产的SensorTag和三星公司生产的GALAXY S5 对系统进行了实现。我们采用Java语言在Android平台上实现了FootStep-Tracker 应用程序。其可以实时的显示SensorTag采集到的传感器数据、用户的步数、用户的行为轨迹以及用户在室内地图中的位置。

\section{展望}

随着物联网的兴起、可穿戴设备和室内定位技术的迅猛发展，带给了人们越来越多生活便利、消费需求与商业机会。虽然已经有很多学者研究如何进行精确的室内定位，也提出了很多高效的解决方案。然而，室内定位技术的实际推广和应用还存在较多的问题，例如如何减少辅助定位设施的部署成本，如何提高定位精度等。本文针对无锚节点的室内定位系统进行了研究，提出了基于脚步感知的无锚节点室内定位系统与系统实现。虽然本文提出的整套室内定位系统经过了理论和真实环境的认证，但仍然存在若干不足，今后将在现有基础上做更为深入的研究和实践，主要有一下几个方面：

\begin{enumerate}
  \item 行为分类器准确性问题。行为分类器部分依赖数据挖掘算法，缺乏逻辑上的区分原理支持。在缺乏大量训练的情况下，对不同用户的分类效果不同。
  \item 方向估算粒度问题。目前的方向估算有效粒度较粗，在采用小于90度的区分粒度时，方向估算具有一定的误差。
  \item 依赖室内环境问题。本系统依赖于室内拓扑环境，在室内环境较为复杂的情况下，系统可以快速收敛过滤掉不可能的用户位置。而在室内环境较为简单的情况下，本系统所计算出的行为轨迹无法确定为绝对的用户位置，在极端情况下甚至无法完成定位。
\end{enumerate}
